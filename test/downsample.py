...
from network.networks import Generator, Downsampler
...
if __name__ == '__main__':
    ...
    eval_dst = PUNET_Dataset_Whole(data_dir='../../PU-GAN_data/test_gt/')
    ...
    for itr, batch in enumerate(eval_loader):
        name = names[itr]
        points = batch[:,:8192,0:3].permute(0,2,1).float().cuda()#cuda
        preds = model(points)
